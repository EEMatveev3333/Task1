...
class AccountUndo {
    ...
    void testUndoNameChange() {
        ...
        System.out.println(account.toString());
        account.setOwnerName("Anatoli SIV");
        System.out.println(account.toString());

        System.out.println(account.canUndo());
        assertTrue(account.canUndo());     //признак счет есть
        account.undo();
        System.out.println("после первого отката " +account.toString());  // Должно быть Anatoli
        account.undo();
        System.out.println("после второго отката  ="+ account.toString());  // должно быть TOLI
