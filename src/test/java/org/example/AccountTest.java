...
class AccountTest {
    ...
    void testUndoCurrencyChange() {
        ...
        System.out.println(account.canUndo());
        account.setCurrencySaldoPair( Currency.RUB, 200);
        System.out.println(account.canUndo());
        System.out.println("счет до отката  " +account.toString());  // должны быть рубли
        account.setCurrencySaldoPair( Currency.USD, 100);
        System.out.println("счет до отката  " +account.toString());  //должны быть доллары и рубли
        account.setCurrencySaldoPair( Currency.USD, 1000);  // меняем 100 доларов на 1000
        ...
        assertFalse(account.containsKeyCurr(Currency.USD));    //проверяем что долларов нет
        ...
        assertFalse(account.containsKeyCurr(Currency.RUB));    //откат  долларов выполнен
        assertFalse(account.canUndo());  //остался голый счет
